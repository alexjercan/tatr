...
static boolean priority_parse(const Aids_String_Slice *slice, unsigned int *priority) {
    long value;

    if (!aids_string_slice_atol(slice, &value, 10) || value < 0 || (unsigned long)value > UINT_MAX) {
        return false;
    }
...
static Aids_Result task_deserialize(Aids_String_Slice buffer, Task *task) {
    ...
    if (!priority_parse(&priority_slice, &task->meta.priority)) {
        aids_log(AIDS_ERROR, "Invalid priority in TASK.md: expected a non-negative number");
        return_defer(AIDS_ERR);
    }
    aids_string_slice_skip_while(&buffer, isspace);
...
static int main_new(const Tatr_Context *ctx) {
    ...
    if (priority_str != NULL) {
        ...
        if (!priority_parse(&priority_slice, &task.meta.priority)) {
            aids_log(AIDS_ERROR, "Invalid priority '%s': expected a non-negative number", priority_str);
            return_defer(1);
        }
    }
...
static Aids_Result task_load(const Aids_String_Slice *task_file_path, Task *task) {
    ...
    if (task_deserialize(serialized_task, task) != AIDS_OK) {
        aids_log(AIDS_ERROR, "Failed to deserialize task '" SS_Fmt "': %s",
                SS_Arg(*task_file_path), aids_failure_reason());
        AIDS_FREE(serialized_task.str);
        return_defer(AIDS_ERR);
    }
...
// Resolves a task HUID and verifies that the task exists.
...
static int main_ls(const Tatr_Context *ctx) {
    ...
    for (size_t i = 0; i < project_dirs.count; ++i) {
        ...
        if (load_tasks_from_dir(&full_tasks_dir, &pt.tasks, sort_by) != AIDS_OK) {
            aids_log(AIDS_ERROR, "Failed to load tasks from '" SS_Fmt "': %s",
